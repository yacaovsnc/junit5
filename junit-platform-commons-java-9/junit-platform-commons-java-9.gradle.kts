...
tasks {
	...
	val generateIntegrationTestsJar by registering(Jar::class) {
		dependsOn(compileTestJava)
		archiveName = "junit-commons-integration-tests.jar"
		from(compileTestJava.get().destinationDir)
		include("integration/**/*")
	}

	// Copy runtime dependencies to a single directory.
	val generateDependenciesDirectory by registering(Copy::class) {
		into("$buildDir/deps")
		from(configurations.testRuntimeClasspath)
	}

	val testScanModulePath by registering {
		...
		doLast {
			...
			val modulePath = files(
					generateDependenciesDirectory.get().destinationDir,
					generateIntegrationTestsJar.get().archivePath
			).asPath
	...
	val testNoJavaScripting by registering {
		dependsOn(generateDependenciesDirectory, generateIntegrationTestsJar)
